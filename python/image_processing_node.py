...
from features_extraction.features_extraction_rg import points_extraction
...
class image_processing:
    ...
    def __init__(self):
        ...
        self._subscriber = rospy.Subscriber("/image_raw/compressed", CompressedImage, self.callback_compressed,
                                            queue_size=1)

        self._subscriber2 = rospy.Subscriber("/image_raw", Image, self.callback, queue_size=1)

        self._subscriber4 = rospy.Subscriber("/image_processing/reference_features", VisualFeatures,
                                             self.callback_ref_sequence_features, queue_size=1)

        # ROS parameter
        self.depth = rospy.get_param("/image_processing/depth", default=4.0)  # 1.85
    ...
    def callback_ref_sequence_features(self, msg):

        ref_sequence_features = np.array([])

        # Take the reference features as given by ReGoVis (in normalized coordinates)
        # To be able to show them, convert in pixel units
        for feature in msg.features:
            ref_sequence_features = np.append(ref_sequence_features, np.array([
                feature.x * self.intrinsic['fx'] + self.intrinsic['cx'],
                feature.y * self.intrinsic['fy'] + self.intrinsic['cy']
            ]))

        self.points_ex.set_ref_sequence(ref_sequence_features)
    ...
    def feature_extractor(self, image):
        ...
        if self._first_publish:
            self._subscriber3 = rospy.Subscriber("/desired_visual_features", VisualFeatures,
                                                 self.callback_desired_features, queue_size=1)
